...
- **OpenCode Compatibility** - Full port to OpenCode IDE
  - Single repository works with both Claude Code and OpenCode
  - Build system that converts markdown sources to OpenCode plugin
  - npm package `opencode-cc10x` for global installation
  - Cross-platform support (Windows, Linux, macOS)
  - Command palette integration with 6 cc10x commands
  - Memory persistence using `.claude/cc10x/` directory
  - 100% feature parity with Claude Code version

### Changed

- Repository structure: Added `src/`, `dist/`, `commands/`, `build/`, `package.json`, `opencode.json`
- Build system: TypeScript compilation + markdown-to-TypeScript conversion
- Installation: Both marketplace (Claude Code) and npm (OpenCode) options
- Commands: Available via router (Claude Code) and command palette (OpenCode)

### Notes

- All agent and skill names remain identical between versions
- Same memory format and persistence mechanism
- Compatible with existing Claude Code installations
- Requires Node.js 18+ and Bun 1.0+ for building
