# cc10x - Intelligent Orchestration System

**The Intelligent Orchestrator for Claude Code and OpenCode**
...
```bash
# Install globally
bun add -g opencode-cc10x

# Or with npm
npm install -g opencode-cc10x
```
...
- Creates plugin file in OpenCode plugins directory
- Adds cc10x commands to OpenCode commands directory
- Configures permissions in OpenCode settings
- Sets up memory directory (`.claude/cc10x/`)

### Usage

1. **Restart OpenCode** if it's running
2. Press `/` to open the command palette
3. Type `cc10x` to see available commands:
   - `/cc10x-orchestrate` - Full orchestration with intent detection
   - `/cc10x-build` - Build features with TDD
   - `/cc10x-debug` - Debug issues with log-first approach
   - `/cc10x-review` - Code review with 80%+ confidence
   - `/cc10x-plan` - Create comprehensive plans
   - `/cc10x-router` - Direct router access

### Example

```
/cc10x-orchestrate "Build a user authentication system with JWT tokens"
```
...
```
cc10x/
├── plugins/cc10x/           # Original Claude Code plugin format
...
│   └── skills/              # 12 skill definitions (markdown)
│       ├── cc10x-router/
│       ├── session-memory/
...
├── src/                     # OpenCode plugin source (generated)
│   ├── agents.ts           # Auto-generated from markdown
│   ├── skills.ts           # Auto-generated from markdown
│   ├── index.ts            # Plugin entry point
│   ├── router.ts           # Router skill
...
│   └── cc10x-router.md
├── build/
│   └── convert-markdown-to-ts.ts  # Build script
...
<p align="center">
  ...
  <em>The Intelligent Orchestrator for Claude Code & OpenCode</em>
</p>
